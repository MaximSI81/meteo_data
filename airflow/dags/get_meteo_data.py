...
def get_data_openweathermap(**context):
    ...
    df_temp = pd.DataFrame([data['main']])  # создаем dataframe погода
    df_wind = pd.DataFrame([data['wind']])  # создаем dataframe ветер
    df_temp.insert(0, 'date', context["date_time"])  # добавляем дату и время запроса
    df_temp[['deg', 'speed', 'gust']] = df_wind[['deg', 'speed', 'gust']]  # объеденяем в общий df  
    df = df_temp[['date', 'temp', 'feels_like', 'temp_min', 'temp_max', 'pressure', 'deg', 'speed', 'gust',]]
    try:
        responce_minio = client_minio.get_object(bucket_name = "prod", object_name = f"{context['city_name']}_{context['date']}_temp.parquet") # получаем файл из minio если существует
        ...
        print(f"Файл {context['city_name']}_{context['date']}_temp.parquet не найден - {error}")
    ...
    client_minio.put_object(
        ...
        object_name = f"{context['city_name']}_{context['date']}_temp.parquet",
