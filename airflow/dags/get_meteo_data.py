...
def get_data_openweathermap(**context):
    ...
    for city in context['city_name']:
        ...
        df['clouds'] = data['clouds']
        df['gust'] = pd.NA if 'gust' not in data['main'] else df['gust']
        df['rain'] = pd.NA if 'rain' not in data else data['rain']
        df['snow'] = pd.NA if 'snow' not in data else data['snow']
        df.insert(0, 'date', context["date_time"])  # добавляем дату и время запроса
        try:
            responce_minio = client_minio.get_object(bucket_name = "prod", object_name = f"{city}_{context['date']}_temp.parquet") # получаем файл из minio если существует
            ...
            print(f"Файл {city}_{context['date']}_temp.parquet не найден - {error}")
        ...
        client_minio.put_object(
            ...
            object_name = f"{city}_{context['date']}_temp.parquet",
